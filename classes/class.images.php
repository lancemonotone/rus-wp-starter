...
class Images {
    ...
    public static function get_image( array $args ): string {
        ...
        [ $id, $have_resized, $size, $class, $original, $resized, $resized_webp, $resized_url, $resized_mime, $resized_webp_url, $resized_webp_mime, $srcset ] = array_values( $image );
    ...
    public static function get_image_tag( array $args ): string {
        ...
        [ $url, $width, $class, $alt, $height, $resized_url, $resized_mime, $resized_webp_url, $resized_webp_mime, $srcset ] = array_values( $parsed );
        ...
        $picture_tag .= '<img loading="lazy" class="' . $class . '" src="' . $url . '" alt="' . $alt . '" width="' . $width . '" height="' . $height . '" ' . ( $srcset ? 'srcset="' . $srcset . '"' : '' ) . '>';
        $picture_tag .= '</picture>';
     ...
     */
    private static function get_image_attributes( array $args ) {
        $parsed = wp_parse_args( $args, [
            'id'     => 0,
            'size'   => 'full',
        ...
        [ $id, $size, $width, $class, $force, $srcset ] = array_values( $parsed );
